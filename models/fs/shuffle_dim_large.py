...
class shuffle_dim_large(nn.Module):
    def __init__(self, args, features,  unique_values ):
        super(shuffle_dim_large, self).__init__()
        
        init_dim = 1
        self.max_emb_dim = 128
        self.embedding = nn.ParameterDict({fea: DynamicEmbedding(unique_values[i],init_dim, self.max_emb_dim )  for i, fea in enumerate(features)})
        ...
        self.weight = nn.ParameterDict({fea: AdaptWeight(args.adapt_dim,init_dim, self.max_emb_dim )  for fea in features})
    ...
    def update_dim_count(self):
        ...
        for fea in self.features:
            ...
            if (gate[:dim] > high).all() and self.args.budget > self.current_dim and dim < self.max_emb_dim:
    ...
    def forward(self, inputs,**kwargs):
        # input is the original fea, not emb
